...
public ResponseEntity<?> adminlogin(@RequestBody JwtRequest authenticationRequest) throws Exception {
    ...
    public ResponseEntity<?> tempLogin(@RequestBody JwtRequest authenticationRequest) throws Exception {
        ...
        if (ue != null) {
            // Check if user has SUPER_ADMIN, ADMIN, HC, or regular user role and set accordingly
            if (ue.getUserRole() != null && ue.getUserRole().contains("SUPER_ADMIN")) {
                System.out.println("User is SUPER_ADMIN, setting logAs to super_admin");
                ue.setLogAs("super_admin");
            } else if (ue.getUserRole() != null && ue.getUserRole().contains("ADMIN")) {
